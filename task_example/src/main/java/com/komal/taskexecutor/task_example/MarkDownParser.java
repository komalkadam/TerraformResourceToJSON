 ...
 public class MarkDownParser {
	 ...
	 private static void parseAndGetAttributeList(List<String> attributeList, String provider_type, String released_version,
		 ...
		 try {
			 ...
			 int attributeListIndex = findAttributeNodeIndex(doc, argumentNodeNumber);
	 ...
	 private static int findAttributeNodeIndex(Document doc, int argumentNodeNumber) {
		 for (int i = argumentNodeNumber + 1; i < doc.jjtGetNumChildren(); i++) {
			 Node node = doc.jjtGetChild(i);
			 if (node instanceof org.tautua.markdownpapers.ast.List)
				 return i;
		 }
		 return -1;
	 }
